...
router.post('/login', (req, res) => {
    ...
    User.findOne({ email: req.body.email })
        .then((findedUserEmail) => {
            console.log('here finded user by email', findedUserEmail); //findedUserEmail prend les infos de user ou null
            if (!findedUserEmail) {
                res.status(200).json({
                    message: '0',
                });
        ...
        .then((resultAfterPwd) => {
            console.log('here after compare pwd', resultAfterPwd);//resultAfterPwd prend true ou false
            if (!resultAfterPwd) {
                res.status(200).json({
                    message: '1',
                })
